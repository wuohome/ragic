...
const SC = {
    ...
    // ── 見紅休「已取消」標記（存在 IS_AUTO 欄位 1000966）──
    // 新制下取消某天見紅休（例如六日去當助教）不刪紀錄，而是標記為已取消。
    // 若直接刪除，「刻意取消」與「還沒預排過」在資料上都是「沒有紀錄」，無法區分，
    // 重跑「一鍵預排休假」時會把取消掉的那天又排回來，造成上班日被誤記為休假。
    GOV_REST_CANCELLED: "見紅休已取消",
    ...
    // ── 清運免排名單（2026-10 起生效，2026-09-10 Joan 拍板）──
    // 理由：垃圾要留到 20:00 才丟，這五位內勤 19:00 前就下班，排了也倒不了。
    // 這是「固定姓名清單」不是部門判斷，也**不是**見紅休名單（GOV_REST_NAMES）——
    // 兩者剛好有四人重疊純屬巧合，見紅休管的是週末/國定假日休假，本清單管的是清運時段，
    // 概念與生效條件都不同，不可互相引用或合併，人員異動一律各自逐一決定。
    // 韓珊珊是排班制、不在見紅休名單，但同樣 19:00 前下班，故列入本清單。
    // 只影響「清運」（🗑️），不影響樓層值日、值班、休假。
    TRASH_DUTY_EXEMPT_NAMES: ["張瓊安", "韓珊珊", "沈郁雯", "呂鴻墀", "王芳瑜"],

    // 清運免排制生效起始年月（YYYY/MM，字串比較，跨年正確）。此月起自動預排不再排到上列五人。
    TRASH_EXEMPT_START: "2026/10",
...
// ── 清運免排判斷（2026-10 起生效，2026-09-10 Joan 拍板）──
// true = 該人在該年月不應被排清運值日（內勤 19:00 前下班、垃圾 20:00 才丟）。
// 2026/09 及以前一律回 false，維持舊行為、不動歷史資料。
// 只給「清運」用；樓層值日／值班／休假不得呼叫本函式。
SC.isTrashExemptFor = (name, year, month) =>
    `${year}/${String(month).padStart(2, "0")}` >= SC.TRASH_EXEMPT_START
    && SC.TRASH_DUTY_EXEMPT_NAMES.includes(name);
